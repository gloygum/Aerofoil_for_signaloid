...
This example considers the lift of an aerofoil moving through air at a constant, subsonic speed.  A Pitot tube measures the static and total pressure of the airflow over the aerofoil and from these measurements, the Mach number (ratio of the airflow speed to the speed of sound) and the lift force exerted on the aerofoil are derived. The primary aim of the calculation is to track uncertainties in the pressure measurements, as well as in simple geometric properties of the aerofoil, through to the final predictions of Mach number and lift.
...
Fluctuations in the Pitot pressure, $p'$, will be correlated with those in the observed static pressure to an extent  determined by the properties of the airflow as well e.g. the spacing of the detectors. We therefore write
$$p' = s (X_p - \bar{p} + \bar{p'}) + (1-s) X_p' + X_{\text{det}}'$$
where $X_p' \sim \mathcal{N}(\bar{p}', \sigma_p)$ and $X_{\text{det}}'$ are further independent random variables modeling fluctuations and detector uncertainties, and where $s\in[0,1]$ is a parameter that describes the degree of correlation between the pressure fluctuations at the two detectors.

We specify the value of the Pitot pressure as  $\bar{p}' = \bar{p} + f_p (p_\text{max}'-\bar{p})$ where $p_\text{max}'$ is the maximum value of $\bar{p}'$ obtained by inverting Eq. (1) with $M=1$ and $p=\bar{p}$, and $f_p$ is the fraction of the maximum pressure difference realised in the set up.
...
The following plot (crated externally) shows example results for $s=1/2$.  The lift is plotted as a function of Mach number with error bars representing the confidence interval in each quantity.
...
In the limit that the variance introduced by the measurement uncertainties is small compared with that introduced by the intrinsic fluctuations, this evaluates as
...
I couldn't get the correlation between the two pressures to evaluate properly. Despite explicit correlation between the fluctations in $p$ and $p'$, it appears that these quantities remain uncorrelated (i.e $\rho=0$ ) in the calculation.  In the folder `correlation/src`, I've put a simpler example that illustrates the issue more clearly.
  
## User testing comments
  1. If I set verbose = 1 in my code and uncomment the line about "sed" in a prinf instruction on line 154, stdout contains a load of unwanted distributional information.  This might be a bug or that I am unknowingly using some kind of escape character (?).
  2. In the mouse-over for `libUncertainDoubleNthMoment`, no mention is made of these being *central* moments. This had me fooled for a while.
  3. The documention (in generally & for the rrt example) could do with a note saying *where* to look for files created by the code. It took me a while to find that they were under "Data & Sensors". One source of confusion is that the execution area has a "files" tab, but this isn't pertinent to output files.
